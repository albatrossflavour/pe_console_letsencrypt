...
describe 'pe_console_letsencrypt' do
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      it {
        is_expected.to contain_letsencrypt__certonly(os_facts['puppet_server']).with(
          {
      ...
      it {
        is_expected.to contain_file("#{nginx_dir}/certs.conf").with(
          {
            'ensure' => 'file',
          },
        )
      ...
      it {
        is_expected.to contain_file("#{cert_dir}/certs/console-cert.pem").with(
          {
            'ensure' => 'file',
          },
        )
      ...
      it {
        is_expected.to contain_file("#{cert_dir}/private_keys/console-cert.pem").with(
          {
            'ensure' => 'file',
          },
        )
